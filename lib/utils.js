 ...
 * If the captain was auto-subbed out (0 minutes), the vice-captain inherits
 * the captain multiplier (2x, or 3x for Triple Captain).
 * If both captain and vice-captain are subbed out, nobody gets the multiplier.
 * Mutates the players array in place.
 * @param {Array} players - Array of player objects with isCaptain, isViceCaptain, multiplier, subOut
 */
function resolveEffectiveCaptaincy(players) {
    const captain = players.find(p => p.isCaptain);
    const viceCaptain = players.find(p => p.isViceCaptain);

    if (!captain || !captain.subOut) return;
    if (!viceCaptain) return;

    const captainMultiplier = captain.multiplier;
    captain.multiplier = 1;
    captain.isCaptain = false;

    if (!viceCaptain.subOut) {
