#!/usr/bin/env node
/**
 * Smoke test for the Set & Forget walkthrough. Same contract as the other tour
 ...
 * leaves state behind.
 *
 * Serves a PRE-SEASON /api/set-and-forget (no managers), which is the state the
 * page is in for months and where it says "check back after GW1". All 10 steps
 * must still appear, off the demo season.
 *
 * Usage (against `npm run dev` or `npm start` on :3000):
 *   node tests/tour/saf-tour.mjs /tmp/shots 390 844
 *   node tests/tour/saf-tour.mjs /tmp/shots 390 844 --midseason
 *   node tests/tour/saf-tour.mjs /tmp/shots 390 844 --gated
 */
...
/** Pre-season: the endpoint answers, but there is nothing in it yet. */
...
const MIDSEASON_SAF = {
  ...
  managers: MEMBERS.map((m, i) => ({
    ...
    safTotal: 150 - i * 10, actualTotal: 150 - i * 10 - i * 5,
    difference: -i * 5,
  })),
  worstTinkerer: { entryId: MEMBERS[1].entryId, name: MEMBERS[1].name, difference: -5 },
};

const EXPECTED_STEPS = [
  'welcome', 'premise', 'worst', 'table', 'diff', 'sort-diff', 'sorted', 'my-row',
  'footer', 'done',
];
...
async function main() {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (p === '/api/set-and-forget') return route.fulfill(json(MIDSEASON ? MIDSEASON_SAF : PRESEASON_SAF));
  ...
  for (let i = 1; i <= EXPECTED_STEPS.length + 2; i++) {
    ...
    // The rank-change badge is places GAINED, i.e. safRank - actualRank, so
    // green means the tinkering moved you up. Legacy had it the other way round
    // and badged the season's worst tinkerer green.
    //
    // Note this is deliberately NOT "badge agrees in sign with Diff": rank is
    // relative, so a manager can gain points and still lose a place to someone
    // who gained more. Only the arithmetic is invariant.
    if (id === 'sorted') {
      const rows = await page.locator('[data-tour="saf-table"] tbody tr').evaluateAll((trs) =>
        trs.map((tr) => {
          const cells = tr.querySelectorAll('td');
          return {
            saf: (cells[0]?.innerText ?? '').replace(/\s/g, ''),
            act: (cells[1]?.innerText ?? '').replace(/\s/g, ''),
          };
        }),
      );
      for (const r of rows) {
        const m = r.act.match(/^(\d+)(=|[+\u2212-]\d+)$/);
        if (!m) { failures.push(`could not parse rank badge from "${r.act}"`); continue; }
        const actualRank = Number(m[1]);
        const badge = m[2] === '=' ? 0 : Number(m[2].replace('\u2212', '-'));
        const expected = Number(r.saf) - actualRank;
        if (badge !== expected) {
          failures.push(`rank badge is ${badge} but safRank-actualRank is ${expected} (row "${r.saf}" / "${r.act}")`);
        }
      }
    }
  ...
  if (after.seenFlag !== '{"set-and-forget":1}') failures.push(`seen flag not written: ${after.seenFlag}`);
